  ...
  async fetchWithRetry(url) {
    for (let attempt = 0;attempt < this.maxRetries; attempt++) {
      ...
      if (remaining !== null && parseInt(remaining, 10) === 0) {
        ...
        if (wait > 0) {
          console.error(`Rate limit exhausted, sleeping ${(wait / 1000).toFixed(0)}s...`);
          await this.sleep(wait);
          continue;
        }
      }
      if (attempt < this.maxRetries - 1 && (resp.status === 403 || resp.status >= 500)) {
        await this.sleep(2 ** attempt * 1000);
        continue;
      }
      const body = await resp.text().catch(() => "");
      throw Object.assign(new Error(`HTTP ${resp.status}: ${body.slice(0, 500)}`), { status: resp.status });
    }
...
var $parse2 = publicApi.parse;
var $parseAllDocuments = publicApi.parseAllDocuments;
var $parseDocument = publicApi.parseDocument;
var $stringify2 = publicApi.stringify;
var $visit = visit.visit;
var $visitAsync = visit.visitAsync;

// src/config.ts
var CONFIG_DIRS = [".github", "."];
var CONFIG_FILES = [
...
function tryLoadYaml(path2) {
  try {
    return $parse2(readFileSync(path2, "utf8"));
  } catch {
    return null;
  }
}
function tryLoadToml(path2) {
  try {
    return $parse(readFileSync(path2, "utf8"));
  } catch {
    return null;
  }
}
...
function fromRaw(raw) {
  ...
  if (raw.branchOnlyRepos)
    cfg.branchOnlyRepos = raw.branchOnlyRepos;
  if (raw.branchOverrides)
    cfg.branchOverrides = raw.branchOverrides;
  if (raw.refOverrides)
    cfg.refOverrides = raw.refOverrides;
  if (raw.reusablePaths)
    cfg.reusablePaths = raw.reusablePaths;
  if (raw.maxConcurrentApi != null)
    cfg.maxConcurrentApi = raw.maxConcurrentApi;
  return cfg;
}
function getExt(p) {
...
function parseInlineConfig(inline) {
  ...
  try {
    ...
    let yamlResult;
    try {
      yamlResult = $parse2(trimmed);
    } catch {
      try {
        return fromRaw($parse(trimmed));
      } catch {
        return null;
      }
    }
    if (typeof yamlResult === "object" && yamlResult !== null && !Array.isArray(yamlResult)) {
      return fromRaw(yamlResult);
    }
    try {
      return fromRaw($parse(trimmed));
    } catch {
...
function loadConfig(root, pathHint, inlineConfig) {
  let raw = {};
  let source = null;
  if (!pathHint) {
  ...
  if (!source) {
    throw new Error("No configuration found. Provide a config file (update-actions-versions.yaml/yml/json/toml), " + "use --config/--inline-config, or set inline-config in the GitHub Action.");
  }
  const required = [
    "branchOnlyRepos",
    "branchOverrides",
...
class RefResolver {
  ...
  async resolveTarget(ar) {
    const repo = ar.repo;
    if (repo in this.config.branchOnlyRepos) {
      const branches = this.config.branchOnlyRepos[repo];
      for (const branch of branches) {
        const sha3 = await this.getBranchSha(repo, branch);
        if (sha3 === ar.currentRef)
          return [branch, sha3];
      }
      const defaultBranch = branches[0];
      const sha2 = await this.getBranchSha(repo, defaultBranch);
      return [defaultBranch, sha2];
    }
    if (repo in this.config.refOverrides) {
      const ref = this.config.refOverrides[repo];
      const sha2 = await this.getTagSha(repo, ref);
      return [ref, sha2];
    }
    const [tag, sha] = await this.getLatestRelease(repo);
    return [tag, sha];
  }
  buildResult(ar, tag, sha) {
  ...
  async getTagSha(repo, tag) {
    const key = `${repo}@tag:${tag}`;
    const cached = this.shaCache.get(key);
    if (cached)
      return cached;
    const refData = await this.client.get(`${GH_API}/repos/${repo}/git/ref/tags/${tag}`);
    const obj = refData.object;
    if (obj.type === "commit") {
      this.shaCache.set(key, obj.sha);
      return obj.sha;
    }
    const tagData = await this.client.get(`${GH_API}/repos/${repo}/git/tags/${obj.sha}`);
  ...
  async getBranchSha(repo, branch) {
    ...
    if (cached)
      return cached;
    const refData = await this.client.get(`${GH_API}/repos/${repo}/git/ref/heads/${branch}`);
  ...
  async getLatestRelease(repo) {
    ...
    if (cached)
      return cached;
    try {
      const release = await this.client.get(`${GH_API}/repos/${repo}/releases/latest`);
      const tag = release.tag_name;
      const sha = await this.getTagSha(repo, tag);
      this.latestReleaseCache.set(repo, [tag, sha]);
      return [tag, sha];
    } catch {
      const releases = await this.client.get(`${GH_API}/repos/${repo}/releases?per_page=1`);
...
// src/rewriter.ts
var USES_RE = /(uses:\s*["']?[^"'\s#]+)@([^\s"'#]+)/;
var TRAILING_RE = /\s+#\s*\S.*$/;

class FileRewriter {
  static buildUsesLine(repo, pathSuffix, sha, tag) {
    const prefix = pathSuffix ? `${repo}/${pathSuffix}` : repo;
    const line = `uses: ${prefix}@${sha}`;
    return tag ? `${line}  # ${tag}` : line;
  }
  static applyPin(line, repo, pathSuffix, sha, tag) {
...
var USES_RE2 = /uses:\s*["']?([^"'\s#]+)/;
var SHA_RE = /^[0-9a-f]{40}$/;
function parseUsesLine(line, file, lineIndex) {
  const m = USES_RE2.exec(line);
  if (!m)
    return null;
  const value = m[1];
  if (value.startsWith("./"))
  ...
  if (SHA_RE.test(ref)) {
    refType = "sha";
  } else if (ref.startsWith("v") && ref.slice(1).replace(/\./g, "").split("").every((c) => /\d/.test(c))) {
    refType = "tag";
    const numPart = ref.slice(1);
    major = parseInt(numPart.includes(".") ? numPart.split(".")[0] : numPart, 10);
  } else {
    refType = "branch";
  }
  return {
...
`);
  ...
  async collectYamlFiles(dir, acc) {
...
function createProgram() {
  ...
  program2.name("update-actions-versions").description("Update GitHub Action pins to latest release SHAs within the same major version").version("1.0.0").option("-w, --write", "write changes to workflow files", false).option("-m, --apply-major", "apply major version bumps (default: report only)", false).option("-v, --verbose", "increase verbosity (repeat for debug)").option("-q, --quiet", "decrease verbosity (errors only)").option("-c, --config <path>", "path to config file (JSON/YAML/TOML)").option("--inline-config <json|yaml>", "inline config as JSON or YAML string (merged with file config, takes precedence)").option("--print-config", "print effective config and exit", false).helpOption("-h, --help", "display help for command");
  return program2;
}
function logLevel(verbose, quiet) {
  let level = 20;
  level -= verbose * 10;
  level += quiet * 10;
  return Math.max(10, Math.min(40, level));
}
function log(level, currentLevel, msg) {
...
async function run(args) {
  ...
  const root = resolve2(".");
  const { config, source } = loadConfig(root, args.config ?? undefined, args.inlineConfig ?? undefined);
  if (source)
    log(20, level, `config: ${source}`);
  if (args.printConfig) {
...
`);
  ...
  if (!process.env.GITHUB_TOKEN && !process.env.GH_TOKEN) {
    log(30, level, "no GITHUB_TOKEN / GH_TOKEN set — unauthenticated API is limited to 60 req/hr");
  }
  ...
  log(10, level, `checking ${refs.length} reference(s) across ${fileCount} file(s)`);
  const apiClient = new LiveGitHubApiClient({
  ...
  for (const r of results) {
    const display = r.ref.pathSuffix ? `${r.ref.repo}/${r.ref.pathSuffix}` : r.ref.repo;
...
`);
        ...
        const newLine = FileRewriter.applyPin(oldLine, r.ref.repo, r.ref.pathSuffix, r.ref.currentRef, r.tag);
        if (newLine !== oldLine) {
...
`);
          commentsFixed++;
          log(10, level, `       -> comment: ${r.ref.file}:${r.ref.lineIndex + 1}`);
        }
      }
      ...
      if (args.applyMajor && r.newSha) {
        ...
        const lines = content.split(`
`);
        lines[r.ref.lineIndex] = FileRewriter.applyPin(lines[r.ref.lineIndex], r.ref.repo, r.ref.pathSuffix, r.newSha, r.tag ?? undefined);
        await writeFileContent(r.ref.file, lines.join(`
`) + `
`);
        changed++;
...
${changed} file(s) updated.`);
  if (commentsFixed > 0)
    log(20, level, `${commentsFixed} comment(s) fixed.`);
  if (changed === 0 && commentsFixed === 0 && majorBumps > 0 && !args.applyMajor) {
    log(20, level, `
${majorBumps} major-bump(s) available. Review manually or use --apply-major.`);
  }
  return errors2 > 0 ? 1 : 0;
}
