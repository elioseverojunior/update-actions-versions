...
var CONFIG_FILES = ["action-pins.yaml", "action-pins.yml", "action-pins.json"];
var DEFAULT_CONFIG = {
  branchOnlyRepos: { "dtolnay/rust-toolchain": ["stable", "nightly"] },
  branchOverrides: { nightly: "nightly" },
  refOverrides: { "github/codeql-action": "v3" },
  reusablePaths: [
...
function loadConfig(root, pathHint) {
  ...
  if (pathHint) {
    ...
    if (loader) {
      ...
      if (overrides) {
        raw = mergeConfig(raw, fromRaw(overrides));
        source = p;
      }
    }
  } else {
  ...
  buildResult(ar, tag, sha) {
    if (ar.refType === "branch") {
      if (sha === ar.currentRef) {
        return { ref: ar, status: "uptodate", newSha: null, tag, message: `already at ${tag}` };
      }
      return { ref: ar, status: "updated", newSha: sha, tag, message: `${tag} (${sha.slice(0, 12)})` };
    }
    if (sha === ar.currentRef) {
      return { ref: ar, status: "uptodate", newSha: null, tag, message: `already at ${tag}` };
    }
...
function parseUsesLine(line, file, lineIndex) {
  ...
  return { file, lineIndex, raw: value, repo, pathSuffix, refType, currentRef: ref, major };
}

class WorkflowScanner {
  async scan(root) {
    const dirs = [join(root, ".github", "workflows"), join(root, ".github", "actions")];
    const files = [];
...
async function run(args) {
  ...
  const root = resolve3(".");
  const { config, source } = loadConfig(root, args.config ?? undefined);
  if (source)
...
`);
  ...
  const { refs, fileCount } = await scanner.scan(root);
  if (refs.length === 0) {
  ...
  for (const r of results) {
    ...
    if (r.status === "uptodate") {
      ...
      if (args.write && r.tag) {
        const line = r.ref.file;
        const content = await readFileContent(line);
        const lines = content.split(`
`);
        ...
        if (newLine !== oldLine) {
          ...
          await writeFileContent(line, lines.join(`
`) + `
`);
          commentsFixed++;
      ...
      log(20, level, ` MAJR ${display.padEnd(50)}  ${r.message}${note}`);
      if (args.applyMajor && r.newSha) {
        const content = await readFileContent(r.ref.file);
        const lines = content.split(`
`);
        lines[r.ref.lineIndex] = FileRewriter.applyPin(lines[r.ref.lineIndex], r.ref.repo, r.ref.pathSuffix, r.newSha, r.tag);
